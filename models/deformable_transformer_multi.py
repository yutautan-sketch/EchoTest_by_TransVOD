...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, 
                ...
                query_embed=None, 
                class_embed=None, 
                temp_class_embed_list=None, 
                temp_bbox_embed_list=None, 
                last_topk=30, 
                ):
        ...
        for lvl, (src, mask, pos_embed) in enumerate(zip(srcs, masks, pos_embeds)):  # lvl=1
            bs, c, h, w = src.shape  # bs=num_frames
            spatial_shape = (h, w)
            ...
            src_flatten.append(src)     # [bs, C, H, W] → [bs, HW, C]
            mask_flatten.append(mask)   # [bs, HW]

        src_flatten = torch.cat(src_flatten, 1)     # [bs, HW, C]
        mask_flatten = torch.cat(mask_flatten, 1)   # [bs, HW]
        ...
        if self.debugmode:
            checkpoint_pre_ste = time.perf_counter()
            print(f"\n=== Prepared for STE ===")
            print(f"spatial_shapes: {spatial_shape} | spatial_shapes.new_zeros((1, )): {spatial_shapes.new_zeros((1, ))}")
            print(f"spatial_shapes.prod(1).cumsum(0)[:-1]: {spatial_shapes.prod(1).cumsum(0)[:-1]} | level_start_index: {level_start_index}")
        ...
        if self.debugmode:
            ...
            print(f"\n=== Passed STE: {checkpoint_ste-checkpoint_pre_ste:.2f}s ===")
            print(f"src_flatten: {src_flatten.shape}, mask_flatten: {mask_flatten.shape}")
            print(f"-> Memory Clip(memory): {memory.shape}")  # [bs, HW, C]
        ...
        if self.two_stage:
            ...
            query_embed, tgt = torch.split(query_embed, c, dim=1)  # [num_queries, 512] split to [num_queries, 256], [num_queries, 256]
            query_embed = query_embed.unsqueeze(0).expand(bs, -1, -1)  # [bs, num_queries, 256]
            tgt = tgt.unsqueeze(0).expand(bs, -1, -1)  # [bs, num_queries, 256]
            reference_points = self.reference_points(query_embed).sigmoid()  # nn.Linear(d_model, 2) + sigmoid
            init_reference_out = reference_points

        if self.debugmode:
            print(f"\n=== Prepared for STD ===")
            print(f"query_embeds -> query_embed: {query_embed.shape} + tgt: {tgt.shape}")
        ...
        if self.debugmode:
            checkpoint_std = time.perf_counter()
            print(f"\n=== Passed STD: {checkpoint_std-checkpoint_ste:.2f}s ===")
            print(f"hs[-1]: {hs[-1].shape} | inter_references_out[-1]: {inter_references_out[-1].shape}")
        ...
        if self.SeqHQM:
            ...
            # Debug information of processing time
            if self.debugmode:
...
def update_QFH(class_embed, hs, last_reference_out, topk):
    ...
    # Get indexes of topk queries from prob.value
    topk_values, topk_indexes = torch.topk(prob[0], topk, dim=1)  # topk_indexes.shape=torch.Size[num_frames, topk]
    # Collect hs queries for indexes specified by elements of topk_indexes
    hs = torch.gather(hs, 1, topk_indexes.unsqueeze(-1).repeat(1, 1, hs.shape[-1]))  # [num_frames, topk, d_model]
    last_reference_out = torch.gather(last_reference_out, 1, topk_indexes.unsqueeze(-1).repeat(1, 1, last_reference_out.shape[-1]))  # [num_frames, topk, 4]
...
class TemporalQueryEncoderLayer(nn.Module):
    ...
    def forward(self, query, ref_query, query_pos = None, ref_query_pos = None):
        # Basically: ref_query = query, query_pos = ref_query_pos = None
        # self.attention
        q = k = self.with_pos_embed(query, query_pos)
        tgt2 = self.self_attn(q.transpose(0, 1), k.transpose(0, 1), query.transpose(0, 1))[0].transpose(0, 1)
        # if ref_query = query, query_pos = ref_query_pos = None
        # -> tgt2 = self.self_attn(query.transpose(0, 1), query.transpose(0, 1), query.transpose(0, 1))
        ...
        # if ref_query = query, query_pos = ref_query_pos = None
        # -> tgt2 = self.cross_attn(tgt.transpose(0, 1), query.transpose(0, 1), query.transpose(0, 1))
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src, src_spatial_shapes, level_start_index, src_padding_mask=None):
        ...
        # cross-attn(tgt+query_pos, reference_points): See models/ops/modules/ms_deform_attn.py
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, query_pos),
                               reference_points,
